class Oscillator {
    ...
    play() {
        ...
        if (this.iterate%5000 == 0) {
          //  console.log(sample__);
        }
        return sample__;
    }
    ...
    mixx() {
        ...
        for (let c = 0; c < this.SID_CHANNEL_AMOUNT; c++) {
            ...
            this.ctrl = this.REG[this.V[c]+this._WG]; //memory[chnadd + 4];//ctrl = waveform/gate | TODO : set on the fly
            this.wf = this.ctrl & 0xF0;
            this.test = this.ctrl & this.TEST_BITMASK;
            this.SR = this.REG[this.V[c]+this._SR]; //memory[chnadd + 6];//SR = Sustain/Release | TODO : set on the fly
            this.tmp = 0;
            this.AD = this.REG[this.V[c]+this._AD]; //memory[chnadd + 5];
            ...
            this.accuadd = (this.REG[this.V[c]+this._FQ_LO] + this.REG[this.V[c]+this._FQ_HI] *256) * this.clk_ratio;
            if (this.iterate%5000 == 0) {
                //console.log(this.accuadd );
            }
            ...
            if (this.wf & this.NOISE_BITMASK) { //noise waveform
                ...
                this.pw = (this.REG[this.V[c]+this._PW_LO] + this.REG[this.V[c]+this._PW_HI] & 0xF) * 16; //(memory[chnadd + 2] + (memory[chnadd + 3] & 0xF) * 256) * 16;//pw = pulsewidth (+'0')| TODO : set on the fly
            ...
            if (this.REG[this._RC] & this.FILTSW[c]) {
                this.filtin += (this.wfout - 0x8000) * (this.envcnt[c] / 256);
            } else if ((c % this.SID_CHANNEL_AMOUNT) != 2 || !(this.REG[this._TVOL] & this.OFF3_BITMASK)) {
